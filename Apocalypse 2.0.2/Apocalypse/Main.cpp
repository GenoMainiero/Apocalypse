...
void InventoryCheck()
{
	...
	for (int i = 0; i < (playerInv->getSize()); i++)
	{
		...
		{
			cout << "Food: " << playerInv->getItem(1)->getCount() << endl;
		}
		else if (playerInv->getItem(i)->getName() == "water")
		{
			cout << "Water: " << playerInv->getItem(0)->getCount() << "     ";
		}
		else
		{
...
int main()
{
	...
	while (!gameover)
	{
		...
		Connect(clothingStore, "north", radioShack);
		...
		Connect(street, "north", hazard1);
		Connect(hazard1, "west", bank);
		Connect(hazard1, "east", clothingStore);
		...
		if (t == r->Stock.getItem(i)->getName())
		{
			...
			else
			{
				if (r->Stock.getItem(i)->getCount() != 0)
				{
					...
					if (r->Stock.getItem(i)->getName() == "radio")
					{
						radiot = 1;
					}
					else if (r->Stock.getItem(i)->getName() == "tape")
					{
						tapet = 1;
					}
					else if (r->Stock.getItem(i)->getName() == "gloves")
					{
						glovest = 1;
					}
					else if (r->Stock.getItem(i)->getName() == "flashlight")
					{
						lightt = 1;
					}
